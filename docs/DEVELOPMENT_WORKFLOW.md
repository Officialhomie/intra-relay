...
| **x402 adapter (`X402_API_KEY`)**       | Real paid query settlement (`FR-PAY-002/003`) — code is DONE (ADR-011)    | Key set per environment | Unset ⇒ `503 PAYMENT_SERVICE_UNAVAILABLE`, no fabricated verify/receipt. Set the key + `X402_NETWORK` per `docs/PAYMENTS.md`.                                                  |

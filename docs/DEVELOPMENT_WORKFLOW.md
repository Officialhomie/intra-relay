...
- A payment renders as paid **only** in `SETTLED` state, which requires a real
  ...
  (`FR-PAY-003`, `BR-005`). No manual receipt entry. An indeterminate settlement
  (`AUTHORISED`, `SETTLE_INDETERMINATE`) is **not** paid and never becomes paid
  without a verified tx hash.
- An infra failure (facilitator unreachable, invalid `X402_*` config) is
  `PAYMENT_SERVICE_UNAVAILABLE` — never `PAYMENT_FAILED` (that is reserved for a
  genuinely bad agent authorisation) — and must not block free routes or the
  buyer web flow (`FR-PAY-006`, ADR-017).
...
| **Celo ERC-8021 attribution tag**               | Crediting any mainnet transaction on the hackathon leaderboard (`BR-007`)              | Issued at registration  | Set `X402_ATTRIBUTION_TAG` once received — it is recorded on receipts. No tag literal in code.                                                                                                                                                                                                                                                               |
| **x402 adapter (`X402_API_KEY`)**               | Real paid query settlement (`FR-PAY-002/003`) — code DONE, hardened (ADR-011, ADR-017) | Key set per environment | Unset **or misconfigured** ⇒ `503 PAYMENT_SERVICE_UNAVAILABLE` (logged, workflow not blocked), no fabricated verify/receipt. Facilitator unreachable ⇒ `503` (retryable); a `settle` timeout ⇒ `503 PAYMENT_SETTLEMENT_INDETERMINATE` (never re-settled). Set the key + `X402_NETWORK` per `docs/PAYMENTS.md`.                                               |
...
| **AskBots CLI / access**                        | Secondary track (AskBots CLI Growth), external agent reviews                           | Not set up              | Do not stub reviews. Genuine feedback-driven changes are logged in `docs/FEEDBACK_CHANGELOG.md` (+ the TS mirror); AskBots rounds append there once the CLI and account exist.                                                                                                                                                                               |
...
| **Amplitude analytics (`*_AMPLITUDE_API_KEY`)** | Pilot observability — how humans behave (M9.5, ADR-021)                                | Keys not created        | `NEXT_PUBLIC_AMPLITUDE_API_KEY` (browser) + optional `AMPLITUDE_API_KEY` (server, forwards 5 actor-less events). Both unset ⇒ the analytics layer is a silent no-op; the audit trail + `/api/pilot/funnel` scorecard are unaffected. Dev stays OFF unless the browser key is set locally. Amplitude-UI verification is manual — see `docs/ANALYTICS.md` §17. |

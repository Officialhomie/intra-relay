...
describe("product type and minimum order", () => {
  it("keeps explicit product support and product-specific quantities three-state", () => {
    expect(evaluateProductType("banners", ["flyers", "banners"])?.status).toBe("MATCH");
    expect(evaluateProductType("banners", ["flyers"])?.status).toBe("NO_MATCH");
    expect(evaluateProductType("banners", null)?.status).toBe("UNKNOWN");
    expect(evaluateMinimumOrder(500, "banners", { banners: 100 })?.status).toBe("MATCH");
    expect(evaluateMinimumOrder(100, "banners", { banners: 100 })?.status).toBe("MATCH");
    expect(evaluateMinimumOrder(50, "banners", { banners: 100 })?.status).toBe("NO_MATCH");
    expect(evaluateMinimumOrder(500, "banners", null)?.status).toBe("UNKNOWN");
  });
});

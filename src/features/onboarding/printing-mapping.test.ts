...
function service(
  ...
  return { label, pricingModel, priceText: null, notes: null, minimumOrder: null };
}
...
describe("mapMinimumOrders", () => {
  it("keeps a numeric minimum associated with its canonical product", () => {
    expect(mapMinimumOrders([{ ...service("Banners"), minimumOrder: 100 }])).toEqual({
      banners: 100,
    });
  });
});
